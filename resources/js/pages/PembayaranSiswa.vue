  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    <form @submit.prevent="submitPayment" class="p-4 space-y-4">
      ...
      <div class="rounded-md border border-gray-200 p-3">
        ...
        <div class="grid gap-2">
          <div
            ...
            :key="item.biaya_sekolah_id"
            class="flex items-center border border-gray-200 rounded-lg p-2 bg-white"
          >
            <div class="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div class="min-w-0">
                <p class="font-semibold text-gray-700 truncate text-sm">
                  {{ item.kategori }}
                </p>
                <p class="text-[11px] text-gray-500">
                  {{ format(item.nominal_tagihan) }} / bln • Sisa :
                  <span class="text-red-600 font-semibold">
                    {{ item.outstanding }} bln
                  </span>
                </p>
              </div>
            </div>

            <div class="flex items-center gap-3 bg-gray-50 p-2 rounded-md border border-gray-200 ml-2">
        ...
        <div class="px-4 pb-4 space-y-2 border-t border-gray-100 pt-3 bg-gray-50">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="h-9 w-9 bg-gray-100 rounded-md flex items-center justify-center text-gray-600 font-bold">
                {{ item.kategori[0] }}
              </div>
              <div>
      ...
      <div class="sticky bottom-0 bg-white border-t border-gray-200 p-4 flex justify-between items-center">
        ...
        <button
          type="submit"
          class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-md text-sm"
        >
          Simpan Pembayaran
        </button>
      </div>
  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    <table class="w-full text-left text-sm">
      ...
      <tbody class="divide-y divide-gray-100">
        ...
        >
          <td class="px-4 py-2">{{ p.tanggal_bayar }}</td>
          <td class="px-4 py-2">{{ p.siswa.nama }}</td>
          <td class="px-4 py-2">{{ p.siswa.kelas }}</td>
          <td class="px-4 py-2">{{ p.metode }}</td>
          <td class="px-4 py-2 font-semibold">{{ format(p.total_bayar) }}</td>
        </tr>
      </tbody>
    </table>
    <!-- PAGINATION -->
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-t border-gray-200 bg-white">
  ...
  <div class="flex items-center gap-2">
    <button
      ...
      class="px-3 py-1.5 border border-gray-300 rounded-md text-xs font-medium transition"
